...
def test_extract_comments_empty():
    tsl_script = ""
    parser = TSLParser(tsl_script)
    assert parser.extract_comments() == []

def test_extract_scenarios_empty():
    tsl_script = ""
    parser = TSLParser(tsl_script)
    assert parser.extract_scenarios() == []

def test_extract_test_cases_empty():
    tsl_script = ""
    parser = TSLParser(tsl_script)
    assert parser.extract_test_cases() == []
